...
(function($) {
  ...
  $.fn.sequencer2 = function(options, cb) {
    ...
    $("<div class='jquery-sequencer2-preload'></div>").appendTo("body").css("display", "none");

    $(paths).each(function() {
      $("<img>").attr("src", this).load(function() {
        $(this).appendTo("div.jquery-sequencer2-preload");
        load++;
        if (load === paths.length) {
          cb();
        }
    ...
    $(window).scroll(function() {
      ...
      index = Math.round(percentageScroll / 100 * options.count);
      if(index < options.count) {
        $("img.sequencer2").attr("src", paths[index]);
      }
    });
